...
|---|---|---|---|---|---|
| 4.1.243 | 2026-07-17 | Pools + Pistol Whip / native VDXR; Eleven Table Tennis + DiRT Rally 2 / OpenComposite → VDXR / Quest 3 | Contracts, full build, installed hash, PID-bound runtime telemetry, headset and game-specific | Frame-level projection context across split-eye, array, translated split-eye and layered side-by-side-atlas menu/cockpit topologies | **Passed:** user confirmed fused overlays with no Pools ghosting and no regressions in the other three titles. Populated releases always retained two projection views; zero locate misses, stale layouts or failed submissions. DiRT menu and in-car states both passed, including ordered projection appended after two application quads. |
...
| 4.1.209 | 2026-07-13 | Pistol Whip / Virtual Desktop VDXR / Quest 3 | Contracts, full build, installed-payload hash proof, headset and game-specific | Visor, calibration, HUD/Trace, notifications, racing presentation tests | **Failed headset validation:** visor and calibration absent; only deviation graph reliable; Alarm-only remained visible; HUD wrapped/scaled incorrectly; notification animation stepped; racing test buttons produced no visible cue. Installed UI, broker and both native DLLs matched the 4.1.209 build. |
| 4.1.210 | 2026-07-13 | Repository and packaged payload | Contracts, policy/history/iRacing/Topmost fixtures, full build and MSI extraction | Demand-based backend, direct calibration/grid, graph/trace, single-row HUD, notification animation, racing presentation overrides | Passed deterministic tests and WPF, broker, signed identity, x64/Win32 native, MSI and fresh payload validation with zero warnings/errors. Pistol Whip headset verification is mandatory before DiRT. |
| 4.1.220 | 2026-07-14 | Installed DiRT Rally 2.0 / OpenComposite | Live process/file inspection plus user-operated post-exit ViewLab UI | Shutdown-independent checkpoint and Open Last Session Graph | **Passed:** `latest.csv` appeared and grew during the game; after DiRT Rally exited, the user opened ViewLab and confirmed the completed session's latency graph remained accessible. GPU alarm colour/recovery visuals were not part of this first-run pass. |
| 4.1.220 | 2026-07-14 | DiRT Rally 2.0 increased-resolution pressure run / OpenComposite | Continuous trace monitoring, post-exit file proof, diagnostic correlation and user-supplied installed-UI screenshot | Full-session boundaries, alarm-only behaviour, cadence recovery, durable rendered graph | **Trace/UI/alarm passed:** sequence 1–87,268 was continuous for 1,306.451 s; 9,058,539-byte output stayed stable after exit. The user observed alarm-only hiding/recovery behaving correctly. The correlated driving interval averaged 11.112 ms (P99 12.042 ms); sampled GPU peaked at 90.42%. PresentMon produced no CSV, so measured presentation-drop evidence remains unavailable. |
| 4.1.224 | 2026-07-15 | Repository, isolated desktop fixtures and extracted MSI | Contracts, full deterministic suite, target-exit integration, ETW-session inspection, WPF/broker/native builds and MSI extraction | DiagMon automatic finalisation, PresentMon streaming/clean stop, early module capture, fixture isolation, pinned collector packaging | **Automated/build passed:** target exit finalised without a `.partial` remainder, the output pump durably retained rows and errors, no `ViewLab-*` ETW session remained, every deterministic suite passed, and extracted PresentMon plus MIT notice matched pinned repository hashes. Sustained real-game CSV and live WPR Trace-cap behaviour remain unverified. |
