...
Assert-Contains 'dllmain.cpp' 'g_releaseDrewViewLabBatchThisFrame\.exchange\(false\)' 'late direct draw is fallback-only after release-time ViewLab batch'
Assert-Contains 'dllmain.cpp' 'DrawCapturedProjectionTextures\(true, true, "direct-fallback"\)' 'late fallback draws the full ViewLab batch (visor then overlays)'
...
# Item 20: visor must be a uniform hard colour; overlays must render above it in all paths.
Assert-Contains 'dllmain.cpp' 'float4 main\(float alpha : ALPHA, float3 color : TEXCOORD0\) : SV_TARGET \{ return float4\(visorColor\.rgb, 1\.0f\); \}' 'visor pixel shader emits the exact configured RGB with opaque alpha'
Assert-Contains 'dllmain.cpp' 'OMSetBlendState\([\s\S]*visorAntialiasing [?] g_d3d11Mask\.bs : g_d3d11Mask\.bsOpaque' 'visor draw uses the opaque blend path when anti-aliasing is disabled'
Assert-Contains 'dllmain.cpp' 'g_d3d11Mask\.bsOpaque' 'visor blend state is the blend-disabled (opaque) state'
Assert-Contains 'dllmain.cpp' 'DrawVisorBorderToTexture\(p\.tex, p\.arrSize, p\.format, ev, p\.projectionViews, rtv\)[\s\S]*?DrawCalibrationPatternsToTexture\(p\.tex, p\.arrSize, p\.format, ev, p\.projectionViews, rtv, true, true\)' 'direct fallback draws visor before overlays'
Assert-Contains 'dllmain.cpp' 'if\(maskEnabled\) DrawVisorBorderToTexture\(tex,2,g_topmostLayer\.format,eyes\[i\],eyes,rtv\);[\s\S]*?DrawCalibrationPatternsToTexture\(tex,2,g_topmostLayer\.format,eyes\[i\],eyes,rtv,false,true\)' 'ordered/topmost path draws visor before overlays'
Assert-Contains 'dllmain.cpp' 'if \(maskEnabled && IncludesMirrorFeature\(obsMirrorVisibilityMask, MirrorVisor\)\)[\s\S]*?DrawVisorBorderToTexture\(g_obsMirrorTexture,[\s\S]*?DrawCalibrationPatternsToTexture\(g_obsMirrorTexture' 'OBS mirror draws visor before overlays'
Assert-Contains 'dllmain.cpp' 'if \(maskEnabled && !g_featurePresentationPlan\.drawDirectVisor\) \{[\s\S]*?DrawVisorBorderToTexture\(copyTex,[\s\S]*?DrawCalibrationPatternsToTexture\(copyTex' 'calibration capture draws visor before overlays'
Assert-Contains 'dllmain.cpp' 'if \(maskEnabled && g_featurePresentationPlan\.drawDirectVisor\)[\s\S]*?DrawVisorBorderToTexture\(tex, arrSize, scFormat, targetViews\[i\], projectionViews,[\s\S]*?DrawCalibrationPatternsToTexture\(tex, arrSize, scFormat, targetViews\[i\], projectionViews' 'release-time direct path draws visor before overlays'
...
Assert-Contains 'dllmain.cpp' 'if\(drewBatch\) g_releaseDrewViewLabBatchThisFrame' 'direct diagnostics track the whole batch, not only the visor'
