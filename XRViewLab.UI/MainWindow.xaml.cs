...
{
    private const string CurrentVersion = "v3.1.0";
    ...
    {
        const string releasesUrl = "https://github.com/Cooooked/xr-viewlab/releases/latest";
        try
        {
            StatusText.Text = "Checking for updates...";
            using var client = new HttpClient();
            client.DefaultRequestHeaders.UserAgent.ParseAdd("XR-ViewLab");
            string json = await client.GetStringAsync("https://api.github.com/repos/Cooooked/xr-viewlab/releases/latest");
            using JsonDocument document = JsonDocument.Parse(json);
            string latest = document.RootElement.TryGetProperty("tag_name", out JsonElement tagElement)
                ? tagElement.GetString() ?? ""
                : "";

            if (string.IsNullOrWhiteSpace(latest))
            {
                StatusText.Text = "Could not read latest release tag.";
                return;
            }
            ...
            StatusText.Text = $"Update available: {latest}. Opening releases.";
            OpenUrl(releasesUrl);
        }
        catch (Exception ex)
        {
