...
#include <cwchar> // wcsstr, swprintf_s
#include <algorithm> // std::find_if, std::swap
#include <functional>
#include <vector>
#pragma comment(lib, "version.lib") // GetFileVersionInfo* for genuine-Valve detection
...
// OpenComposite re-implements OpenVR on top of OpenXR, but aborts the whole process when an
// application requests an OpenVR interface version it does not implement (e.g. IVRCompositor_029,
// IVROverlay_028). Detect it so ReShade can avoid making those requests and fall back gracefully:
//  - DX11/D3D injection: the game's own VR_GetGenericInterface drives the Submit hook (lower version)
//  - OpenXR layer injection: effects are applied in xrEndFrame, so the OpenVR compositor isn't needed
// Returns true only for the GENUINE Valve openvr_api.dll (CompanyName contains "Valve").
// OpenComposite's openvr_api.dll re-implementations have empty / non-Valve version info.
static bool reshade_module_is_valve(HMODULE mod)
{
	wchar_t path[MAX_PATH] = L"";
	if (GetModuleFileNameW(mod, path, ARRAYSIZE(path)) == 0)
		return false;

	DWORD ignored = 0;
	const DWORD size = GetFileVersionInfoSizeW(path, &ignored);
	if (size == 0)
		return false; // no version resource at all -> not the genuine signed Valve loader

	std::vector<uint8_t> data(size);
	if (!GetFileVersionInfoW(path, 0, size, data.data()))
		return false;

	struct lang_codepage { WORD language; WORD codepage; } *translate = nullptr;
	UINT translate_bytes = 0;
	if (VerQueryValueW(data.data(), L"\\VarFileInfo\\Translation", reinterpret_cast<void **>(&translate), &translate_bytes))
	{
		for (UINT i = 0; i < translate_bytes / sizeof(lang_codepage); ++i)
		{
			wchar_t query[64] = L"";
			swprintf_s(query, L"\\StringFileInfo\\%04x%04x\\CompanyName", translate[i].language, translate[i].codepage);
			wchar_t *company = nullptr; UINT company_len = 0;
			if (VerQueryValueW(data.data(), query, reinterpret_cast<void **>(&company), &company_len) && company != nullptr &&
				wcsstr(company, L"Valve") != nullptr)
				return true;
		}
	}
	return false;
...
// Universal OpenComposite detection (any variant, 32- or 64-bit):
//   (1) the in-process openvr_api.dll is NOT the genuine Valve loader (OC ships its own openvr_api.dll), OR
//   (2) it exports 'HmdSystemFactory' (some OC builds), OR
//   (3) OC is the registered runtime: a loaded vrclient(.dll/_x64.dll) resolves to an opencomposite/openovr path.
// This avoids ReShade's IVRCompositor_029 / IVROverlay_028 probes that abort OpenComposite.
bool reshade_is_opencomposite()
{
	if (const HMODULE openvr_module = GetModuleHandleW(L"openvr_api.dll"); openvr_module != nullptr)
	{
		if (GetProcAddress(openvr_module, "HmdSystemFactory") != nullptr)
			return true;
		if (!reshade_module_is_valve(openvr_module))
			return true; // openvr_api.dll present but not genuine Valve => a re-implementation (OpenComposite)
	}

	for (const wchar_t *name : { L"vrclient_x64.dll", L"vrclient.dll" })
	{
		if (const HMODULE vrclient_module = GetModuleHandleW(name); vrclient_module != nullptr)
		{
			wchar_t path[MAX_PATH] = L"";
			if (GetModuleFileNameW(vrclient_module, path, ARRAYSIZE(path)) != 0)
			{
				for (wchar_t *p = path; *p != L'\0'; ++p)
					if (*p >= L'A' && *p <= L'Z')
						*p += 32; // ASCII lower-case in place
				if (wcsstr(path, L"opencomposite") != nullptr || wcsstr(path, L"openovr") != nullptr)
					return true;
			}
		}
	}
...
{
	// Skip forcing the latest compositor interface under OpenComposite — it would abort the process
	// (OpenComposite does not implement IVRCompositor_029). The Submit hook is still installed when
	// the application requests its own (supported) compositor version in VR_GetGenericInterface above.
