...
| `hud_trace_enabled` | 0 | `hudTraceEnabled` | Independently enables the selectable performance graph in both eyes. |
| `topmost_visor_overlays` | 0 | `topmostVisorOverlays` | Experimental live switch. Uses one stable transparent projection swapchain allocation attempt per OpenXR session. Any capacity, creation, rendering, submission, or device-loss failure latches Topmost off for that session and restores the normal eye-texture path; toggling cannot re-arm it. |
