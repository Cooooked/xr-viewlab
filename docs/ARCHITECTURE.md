...
| Automatic topmost overlays | `EnsureTopmostLayer`, `RenderTopmostLayer`, `BlockTopmostLayer`, `DestroyTopmostLayer`, `TopmostSubmission`, `LatchTopmostDemand`; projection-only applications retain direct rendering. A distinct application compositor layer latches Topmost demand for the session. Its transparent two-slice projection swapchain is prepared on the transition frame and appended after application layers on following frames, while direct common-scene drawing is suppressed. Literal-pixel calibration always remains on the game texture. Capacity/runtime/D3D failure latches direct fallback. `overlay_force_direct=1` is the diagnostic escape. |
| Native performance HUD | `HardwareTelemetry.cpp/.h` own a bounded Windows/PDH/DXGI/ICMP worker and immutable snapshot; `NetworkProbe.h` owns deterministic rolling probe maths. `HudWidgetId`, `kHudWidgetRegistry`, `UpdateHudMetrics`, `HudDrawSnapshot`, `DrawCalibrationOverlayToTexture`, and `RenderPolicy.h` consume it. All enabled widgets, including optional PING/LOSS/JIT/NET, pack into one proportional row; scale changes rings, text and spacing together. APP/VR/FPS/frame interval remain per-frame OpenXR timing. Both eyes reuse one draw snapshot. |
| Performance Trace + markers | `hudTraceVisibilityMode`, `TraceVisibilityState`, `UpdateTraceVisibility`, `EffectiveGraphChannels`, `CapturePerformanceTraceMarker`, `SavePerformanceTraceSession`, `PerformanceTrace` and `PerformanceTraceWindow`; Off/Always/Alarm-only changes live drawing only. A marker bind edge uses the current `xrWaitFrame` QPC timestamp, gets a numbered visor acknowledgement, remains part of the real sample stream and is saved to the versioned post-session trace. The full-session recorder is a bounded one-hour ring; the live graph remains a separate 600-sample window over that same source. |
| Visor overlays (boundary/crosshair/notifications/racing events) | `DrawViewLabOverlaysToTexture`, `EnsureNotifyCardTexture`, `ConsumeRacingState`, `AnyViewLabOverlay`, `EvaluateNotificationAnimation`, `kOverlayPS`, `kTexturedPS`; flat overlays use explicit colour and cards use textured quads. The broker publishes notification lifecycle timestamps; native evaluates fade/slide every VR frame and applies alpha once. `RacingStateService` publishes generic spotter/flag/lap semantics plus non-persistent presentation-test flags through its 64-byte mapping. Production enable gates remain unchanged. |
| Clock + OpenXR session timer | `ClockWidget.h::Format` supplies bounded fixed text; `XRViewLab_xrCreateSession`/`XRViewLab_xrDestroySession` own the monotonic session epoch; `DrawViewLabOverlaysToTexture` draws the dedicated two-line card through the common stereo Direct/Topmost path. Local wall time never drives elapsed duration. |
| Sticky note widget | `StickyNote.h::Wrap` owns bounded text normalization/wrapping; `stickyNote*`, the rising-edge key check in `XRViewLab_xrWaitFrame`, and `DrawViewLabOverlaysToTexture` own session visibility and native card drawing. It is one startup-configured note, independent of notification shared memory. |
| OBS recording cue | `ObsRecordingProvider` implements OBS WebSocket v5 authentication and bounded `GetRecordStatus` polling in the broker, then publishes a 16-byte `XRViewLabObsRecordingState` mapping. `ObsRecordingActive` validates a stable generation and `DrawViewLabOverlaysToTexture` draws red corners only for real active-recording state. Because drawing occurs in submitted eye textures, capture exclusion is not an architectural property. |
