...
| Native performance HUD | `HardwareTelemetry.cpp/.h` own a bounded Windows/PDH/DXGI worker and immutable snapshot; `HudWidgetId`, `kHudWidgetRegistry`, `UpdateHudMetrics`, `HudDrawSnapshot`, `DrawCalibrationOverlayToTexture`, and `RenderPolicy.h` consume it. All enabled widgets pack into one proportional row; scale changes rings, text and spacing together. APP/VR/FPS/frame interval remain per-frame OpenXR timing. Both eyes reuse one draw snapshot. |
| Performance Trace visibility | `hudTraceVisibilityMode`, `TraceVisibilityState`, `UpdateTraceVisibility`, `EffectiveGraphChannels`; Off/Always/Alarm-only changes drawing only. Each graph mode always has a compatible useful channel. Alarm-only responds only to alarms represented by selected graph channels, then uses one bounded hold and 500 ms fade. |
...
| Bounded technical history | Broker-owned `HistoryService` atomically replaces `%LOCALAPPDATA%\XR ViewLab\history.jsonl` for low-frequency notification dispositions, telemetry state, racing cues and lap metadata. It has fixed 14-day/512-record/512-KiB limits, no notification-body field, and catches all storage failures. `ClearHistory_Click` sends the broker command; `Tests/HistoryFixtures` verifies malformed input, retention, UTF-8 size, schema limits and clearing. |
