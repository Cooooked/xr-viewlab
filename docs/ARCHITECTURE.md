...
| Overlay live-state authority | `liveOwns(OverlayFeatureId)` + `g_liveAuthoritativeMask` (live-state v14). A per-app profile override is read once at `xrCreateSession` and normally beats live global edits, which meant the per-app editor's preview drags could not show until a game restart. `ProfileWindow.OverlayLivePreview` reports each drag to `MainWindow.PublishProfileOverlayLive`, which publishes the resolved value with that feature's bit set; the layer then applies it over the profile override. Session-setup code that applies a profile's own values still reads the override mask directly. |
| Notification lifetime | `EvaluateNotificationAnimation(now, enterTick, leaveTick, riseMs, leaveMs, durationMs, graceMs)`. The broker owns `leaveTick`; `durationMs` (`notify_duration_ms`) is a layer-side failsafe so a card whose leave never arrives still fades out instead of persisting for the session. The grace margin keeps a healthy broker's own timing authoritative. |
