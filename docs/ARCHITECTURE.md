...
| Native performance HUD | `HudWidgetId`, `kHudWidgetRegistry`, `HudMetricState`, `HudGraphMode`, `XRViewLab_xrWaitFrame`, `XRViewLab_xrBeginFrame`, `XRViewLab_xrEndFrame`, `UpdateHudCadence`, `RecordHudFrameSample`, `RecordHudAppWorkSample`, `UpdateHudMetrics`, `UpdateHudState`, `DrawCalibrationOverlayToTexture`, `HudDrawSnapshot`; modular ordered CPU/GPU/APP/VR widgets plus a bounded channel graph. APP is begin-return→end-entry wall time divided by the cadence-aware budget. VR state is relative to `predictedDisplayPeriod`, distinguishes target/warning/critical/stable reprojection/unstable, and never uses a millisecond whitelist. One immutable snapshot carries metric values, states, widget order/mask, graph selection, and 600 samples to both eyes. See `PERFORMANCE_HUD_REDESIGN.md`. |
