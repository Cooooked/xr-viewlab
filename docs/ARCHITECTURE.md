...
| Visor overlays (boundary/crosshair/notifications/iRacing events) | `DrawViewLabOverlaysToTexture`, `EnsureNotifyCardTexture`, `AnyViewLabOverlay`, `BoundaryFlashActive`, `kOverlayPS`, `kTexturedPS`; flat overlays use an explicit constant-colour shader (never the VDXR-broken vertex-colour interpolant), while cards use textured quads. `CrosshairPreview` and `BeanMaskEditor.DrawCrosshair` mirror native rectangle geometry. The signed medium-integrity `ViewLab.NotificationBroker` owns global Windows consent, `UserNotificationListener`, card composition and the dedicated mapping independently of settings/game lifetime; `NotificationBrokerClient` only controls it. `IRacingTelemetryProvider` validates the public SDK mapping off-thread, consumes only fresh ticks, owns stale/session reset, and normalizes exact spotter, flag and lap semantics through `IViewLabEventProvider`; the renderer has no iRacing dependency. |
