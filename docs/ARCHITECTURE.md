...
| Automatic topmost overlays | `EnsureTopmostLayer`, `RenderTopmostLayer`, `BlockTopmostLayer`, `DestroyTopmostLayer`, `TopmostSubmission`; normal backend owns one transparent two-array-slice OpenXR projection swapchain per session, sized from stable game-texture capacity. First frame remains direct while allocation arms; later direct drawing is suppressed to avoid duplicates. Any capacity/runtime/D3D failure latches the backend off until a new session and restores direct rendering. RTVs use the runtime resource's typed format; the premultiplied target is appended after application layers at `xrEndFrame`. `overlay_force_direct=1` is a session-start diagnostic/profile escape hatch, absent from ordinary UI. |
...
| Performance Trace visibility | `hudTraceVisibilityMode`, `g_hudTraceVisibilityAlpha`, `g_hudTraceHoldUntilTick`; Off/Always/Alarm-only changes drawing only. The fixed timing ring continues collecting in alarm-only while hidden. Existing sustained `HudAlarmState` drives appearance; recovery uses one bounded hold and 500 ms fade. |
