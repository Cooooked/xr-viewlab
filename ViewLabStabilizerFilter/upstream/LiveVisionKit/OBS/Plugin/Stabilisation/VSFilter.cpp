...
//---------------------------------------------------------------------------------------------------------------------
	...
	obs_properties_t* VSFilter::Properties()
	{
		...
		auto property = obs_properties_add_int(
			...
			"Smoothing Radius (higher = steadier, more delay)",
		...
		obs_property_list_add_string(property, "Dynamic (recommended)", MOTION_MODEL_DYNAMIC);
		...
		property = obs_properties_add_list(
			...
			"Unreliable-motion Suppression",
			OBS_COMBO_TYPE_LIST,
			OBS_COMBO_FORMAT_STRING
		);
		obs_property_list_add_string(property, "Off", SUPPRESSION_MODE_OFF);
		obs_property_list_add_string(property, "Strict (recommended)", SUPPRESSION_MODE_STRICT);
		...
		obs_properties_add_bool(
			properties,
			PROP_TEST_MODE,
			"Test Mode (tracking points and timing)"
		);
