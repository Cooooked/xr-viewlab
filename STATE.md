...
**Updated:** 2026-08-23
**Current version:** 4.1.336 — `F:\AI-Projects\ViewLab\dist\ViewLab-4.1.336.msi` (contracts pass; full MSI build 0 warnings / 0 errors; **not live-validated yet**).

**2026-08-24 — 4.1.336 UI layout and popout follow-up.** The enabled-card outline is restored to bright ViewLab red while its fill stays wine-tinted. The four app-list actions use equal Grid columns in the narrow three-column layout, so Show hidden remains on the same row. The rehosted Overlays window imports ViewLab's TextBlock, CheckBox and Slider styles and explicitly colours Expanders, addressing black overlay headings and default light sliders. The dense ViewLab Media Capture inline explanation is removed because it is already covered by the OBS `?` help. The OpenXR API Layers `?` is placed beside the title, not in an empty row above the scope tabs. Contracts and the full MSI build pass. **Pending live validation:** these UI fixes.
