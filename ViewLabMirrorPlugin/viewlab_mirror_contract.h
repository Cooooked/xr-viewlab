...
#define VIEWLAB_MIRROR_VERSION 3u
...
typedef struct ViewLabMirrorSurface {
    ...
    uint32_t eyeMode;         /* mode the producer ACTUALLY published this frame (0 left, 1 right) */
    ...
    uint32_t requestedEyeMode;      /* eye the user selected: 0 left, 1 right */
    ...
    /* v3: consumer->producer. Non-zero means composite the overlay quad layers submitted by
     * the API layers above ViewLab (ReShade's VR menu, OpenKneeboard, RaceLab) into the
     * captured eye. Zero gives the bare projection frame. */
    uint32_t requestedShowOverlays;
