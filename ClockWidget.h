...
struct Text {
    std::array<char, 6> local{};    // HH:MM
    std::array<char, 9> session{};  // HH:MM:SS, capped at 99:59:59
};

inline Text Format(uint32_t localHour, uint32_t localMinute, uint64_t elapsedMilliseconds) {
    Text result{};
    localHour %= 24;
    localMinute %= 60;
    ...
    std::snprintf(result.local.data(), result.local.size(), "%02u:%02u", localHour, localMinute);
